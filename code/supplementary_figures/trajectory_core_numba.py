"""Numba-accelerated finite-pool ABM engine for the full S5.2 design.

supplementary_material_1.tex (Supplementary Material 1, sections S5.1--S5.6 and S7) documents a crossed
...
``suppl_FigS6/s5_common.py`` for the (independently designed, event-count) S5.6
verification -- both draw on the crossed design in supplementary_material_1.tex Table S3 --
...
def build_protocols(U0: int, I0: int) -> list[Protocol]:
    """The 195-protocol crossed design of supplementary_material_1.tex Table S3, at given (U0, I0)."""
    out: list[Protocol] = []
    for R in R_GRID:
        for c in C_GRID:
