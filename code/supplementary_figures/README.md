# Supplementary ABM-validation figures (Supplementary Material 1 / supplementary_material_1.tex)

Two production designs live in this folder:

**S1-S4** reproduce the full crossed design supplementary_material_1.tex documents (sections
S5.1-S5.5): 195 raw-rate protocols (R in {2,4,6}, c in
...
**S5** is the separate S5.6 direct event-level verification: the same
...
(`suppl_FigS6/s5_common.py`) and master seed (20260817).
...
- `S5/` -- `generate_data_S5.py` then `generate_figure_S5.py`, sharing the
  model core in `suppl_FigS6/s5_common.py` (needs `numba`, see `S5/requirements.txt`).
