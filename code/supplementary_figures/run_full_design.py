#!/usr/bin/env python3
"""Production run of the full S5.2 crossed design (supplementary_material_1.tex, Table S3).
...
Master seed 20260804 (supplementary_material_1.tex Table S4 "Global numerical settings").
...
quoted in supplementary_material_1.tex. S2, S3, and S4's generate_data scripts read
pool0_selected_trajectories.npz for their specific representative
protocols (all drawn from pool 0, as supplementary_material_1.tex states), so nothing is
resimulated twice.
...
# U0=8000, R=4, matching the specific rows supplementary_material_1.tex figures S2-S4 display.
NEEDED_SELECTORS = (
...
def main() -> None:
    ...
    elapsed = time.perf_counter() - started
    manifest = {
        "design": "supplementary_material_1.tex S5.2 crossed design (195 raw-rate protocols)",
