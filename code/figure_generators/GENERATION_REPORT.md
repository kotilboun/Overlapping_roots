...
event-level ABM verification scatter plot formerly shown only as Fig. S6
of Supplementary Material 1 was promoted to a full main-text figure and inserted at
...
match. The new `Fig2.pdf` is a copy of `supplementary_figures/suppl_FigS6/
supp_S5_event_term_validation.pdf` (documentation stub in `suppl_FigS6_stub/`, no
...
`figure_generators/maintext_Fig1` through `figure_generators/maintext_Fig5` (`suppl_FigS6_stub`
...
shared_trajectory_engine/trajectory_core.py` and were checked to reproduce Figure 7's
...
```text
maintext_Fig1: python 01_generate_data.py
          python 02_make_figure.py

maintext_Fig2: python 01_generate_data.py --workers 8
          python 02_make_figure.py

suppl_FigS1: python 01_generate_data.py
          python 02_make_figure.py

maintext_Fig3: python 01_generate_data.py --workers 8
          python 02_make_figure.py

suppl_FigS7: python 01_generate_data.py --workers 8
          python 02_make_figure.py

maintext_Fig4: python 01_generate_data.py --workers 8
          python 02_make_figure.py

suppl_FigS8: python 01_generate_data.py --workers 8
          python 02_make_figure.py

maintext_Fig5: python 01_generate_data.py --workers 8
          python 02_make_figure.py
```

`suppl_FigS6_stub` has no `01_generate_data.py`/`02_make_figure.py` of its own -- it
...
see `suppl_FigS6_stub/README.md`.
...
- Figure 2: event-level verification (produced by `supplementary_figures/S5`,
  ...
  removal `1.00010`. See `supplement/supplementary_material_1/supplementary_material_1.tex` Sect. S5.6 for full
...
`../manuscript.pdf` has 48 pages and contains exactly one caption for every
numbered figure:
...
2026-08-06 (Sect. 2.8, promoted from Supplementary Material 1's Fig. S6). Figures
...
```text
python audit_manuscript_figures.py ../manuscript.pdf --render-dir integration_previews
```
