#!/usr/bin/env python3
"""Step 2/2 for Figure 9: draw representative R=4 trajectories vs. the algebraic QSS closure.
...
directly to the manuscript's ``figures/Fig9.pdf`` -- the exact path
``\\includegraphics{figures/Fig9.pdf}`` in manuscript.tex uses -- so
re-running this script is the only step needed to refresh what LaTeX
renders. It never re-simulates the ABM or re-solves the closure.
    ...
    with np.load(summary_path) as summary, np.load(closures_path) as closures:
        ...
        for column, c in enumerate(C_VALUES):
            ...
            for row_index, (variable, ylabel, ylim, yticks) in enumerate(ROW_SPECS):
                ...
                axis.plot(
                    times, closures[f"algebraic_qss0_{variable}_{key}"],
                    color=QSS_COLOR, linestyle=(0, (5, 2.5)), linewidth=1.15,
                    label=r"algebraic QSS, $\widehat m_1^{(0)}$" if column == 0 and row_index == 0 else None,
                )
        ...
        pdf_path = MANUSCRIPT_FIGURES_DIR / "Fig9.pdf"
        fig.savefig(pdf_path, metadata={"Title": "Figure 9: representative R=4 trajectories vs. algebraic QSS closure", "Creator": Path(__file__).name})
        png_path = pdf_path.with_suffix(".png")
        tiff_path = pdf_path.with_suffix(".tiff")
        fig.savefig(png_path, dpi=600)
        fig.savefig(tiff_path, dpi=600, pil_kwargs={"compression": "tiff_lzw"})
        fig.savefig(HERE / "Fig9_preview.png", dpi=200)
        plt.close(fig)
        make_rgb(png_path)
        make_rgb(tiff_path, "tiff_lzw")

    caption = (
        "Fig. 9 Representative finite-pool trajectories for the zeroth-order algebraic QSS closure at "
        "R=4. Columns, rows, ABM summaries, and activation follow Fig. 8 and use the identical R=4 ABM "
        "replicates. Blue curves are the sample ensemble-mean ABM trajectory and pale bands are pointwise "
        "2.5th-97.5th replicate percentiles across 120 independent realizations. The green dashed curve is "
        "the algebraic closure m_1^(0)(R_S)=R_S/(R_S+1) (Eq. 7.6), which assigns m_1 instantaneously from "
        "the susceptibility-adjusted transmission strength R_S(tau) and does not retain tracing-induced "
        "genealogical memory. The vertical dotted line marks activation of forward tracing at tau=0.5. "
        "Simulations use S_0=8000, I_0=160, and 151 observation times on 0<=tau<=5, with "
        "(gamma/Gamma,gamma_c/Gamma,p_f)=(0,1,c). The trajectories shown are the same R=4 ABM ensemble "
        "underlying the E_C errors reported in Fig. 7 and the dynamic-closure overlays in Fig. 8.\n"
    )
    (HERE / "Fig9_caption.txt").write_text(caption, encoding="utf-8")
    print(f"Wrote {pdf_path} (LaTeX-linked) and {HERE / 'Fig9_preview.png'}.")
