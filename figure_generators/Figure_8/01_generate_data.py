#!/usr/bin/env python3
"""Step 1/2 for Figure 8: representative R=4 trajectories vs. dynamic closures.
...
Figure 6/Figure 7, via ``figures8_9_shared/trajectory_core.py``), solves the
depth-K=1,2,3 dynamic tail closures (Sect. 7.1), and writes:

- pointwise ABM ensemble-mean and 2.5th-97.5th replicate-percentile bands
  for i=I/U0, u=U/U0, m1=M1/I, and flux=c*m1*i (what Figure 8 plots);
- the same four closure trajectories, for each dynamic order K;
- decomposed trajectory-error metrics (closure vs. ABM mean, Eq. 7.16, with
  ...
  post-activation window tau>=0.5 -- separating closure inaccuracy from
  sampling noise, as in Sect. 7.6.

Pure data generation -- no matplotlib import. Run:
...
DATA_DIR = HERE / "data"
SHARED = HERE.parents[0] / "figures8_9_shared"
sys.path.insert(0, str(SHARED))

import trajectory_core as core  # noqa: E402

# R_grid_for_seeding reproduces Figure 7's exact (R, c) spawn order so that,
# at R=4, the ABM replicates below are bit-for-bit identical to the R=4 cells
# already simulated for Figure 7 -- verified against Figure 7's cached
# data/source_abm_trajectories.npz in this script's validation step.
R_GRID_FOR_SEEDING = (1.0, 1.5, 2.0, 4.0)
R_VALUE = 4.0
C_VALUES = (0.0, 0.25, 0.5, 0.75, 1.0)
DYNAMIC_ORDERS = (1, 2, 3)
VARIABLES = ("i", "u", "m1", "flux")
INTERVALS = ("full_trajectory", "post_activation")
...
def verify_against_figure6(abm_by_c: dict[float, np.ndarray]) -> bool:
    """Best-effort check: R=4 replicates match Figure 7's cached archive."""
    fig6_archive = HERE.parents[0] / "Figure_7" / "data" / "source_abm_trajectories.npz"
...
def main() -> None:
    ...
    for c in C_VALUES:
        ...
        closure_states = {f"dynamic_K{K}": core.solve_dynamic(R_VALUE, c, K, times, 1.0 / 60.0, U0, I0, tau_on) for K in DYNAMIC_ORDERS}
        closures_z = {name: closure_z(state, U0) for name, state in closure_states.items()}
        for name, state in closure_states.items():
            derived_closure = core.closure_i_u_m1_flux(state, c, U0)
            for variable in VARIABLES:
                closure_payload[f"{name}_{variable}_{key}"] = derived_closure[variable]
    ...
    manifest = {
        ...
        "content": "representative R=4 finite-pool trajectories vs. depth-K dynamic closures",
        "protocol": asdict(protocol),
        "R": R_VALUE,
        "c_values": list(C_VALUES),
        "dynamic_orders": list(DYNAMIC_ORDERS),
    ...
    print(
        f"Generated Figure 8 data for R={R_VALUE:g}, {len(C_VALUES)} c-values, "
